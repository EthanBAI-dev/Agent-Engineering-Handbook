...
> **术语说明**
>
> - **TypedDict**：规定 State 有哪些字段以及各自的数据类型。
> - **Annotated**：在类型之外给 `log` 绑定额外的合并规则。
...
> **术语说明｜Reducer（归并规则）**  
> 规定新数据怎样与旧 State 合并；这里它让新日志追加到旧列表后面，而不是覆盖旧列表。
...
> **术语说明**
>
> - **stream**：逐步返回节点输出，用来观察运行过程。
> - **invoke**：执行一次完整调用，用来取得最终结果。
