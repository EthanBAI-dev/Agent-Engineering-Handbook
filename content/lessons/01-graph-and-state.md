...
其中，TypedDict规定 State 有哪些字段以及各自的数据类型；Annotated在类型之外给 `log` 绑定额外的合并规则。
...
其中，work执行一轮工作并更新计数；review在循环结束后追加复核记录。
...
它规定新数据怎样与旧 State 合并；这里它让新日志追加到旧列表后面，而不是覆盖旧列表。
...
其中，START标记流程从哪里开始；END标记流程在哪里结束。
...
其中，stream逐步返回节点输出，用来观察运行过程；invoke执行一次完整调用，用来取得最终结果。
