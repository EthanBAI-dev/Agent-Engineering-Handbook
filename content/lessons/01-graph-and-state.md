...
> **配图 F01-1｜State 的三个字段，两种命运**
> **形式**：静态示意图 · `assets/lessons/01-state-fields.svg`
> **画什么**：一个方框列出 `topic` / `count` / `log` 三行。前两行右侧标「覆盖」，第三行标「累加」，
> 并用一条细线连到 `Annotated[list[str], operator.add]`。
> **必须标注**：三个字段名；「覆盖」与「累加」的对比；触发累加的那段类型标注。
> **不要出现**：数据库图标、时间轴、模型头像——State 不是存储，也不是记忆。
> **替代文本**：State 有三个字段，topic 和 count 被新值覆盖，log 因为带了 reducer 而累加。
...
> **配图 F01-2｜循环不是能力，是一条边**
> **形式**：流程图 · `assets/lessons/01-conditional-edge.svg`
> **画什么**：`START → plan → work`，work 上方一条弧线绕回自己标「条件成立」，
> 另一条直线到 review 标「条件不成立」，再到 END。判断函数画成 work 与两条边之间的小菱形。
> **必须标注**：两条分支各自的条件；判断发生在 **work 之后**，读的是更新后的 State。
> **不要出现**：把判断画进 work 节点内部——那正是这一课要纠正的误解。
> **替代文本**：work 执行后由条件边判断，成立就回到 work 形成循环，不成立才走向 review。
...
> **互动 U01-1｜每点一次，只走一个节点**
> **状态**：已实现 · `apps/web/src/components/lesson-one-lab.tsx`
> **用户做什么**：点「执行下一步」逐节点推进；跑完后把阈值从 4 改成 6 再跑一遍。
> **屏幕上变什么**：当前 Node 高亮；刚走过的 Edge 高亮；`count` 显示旧值→新值；`log` 追加一项。
> **通过条件**：阈值 6 跑完后，`work` 恰好执行 5 次、`count` 为 6、`log` 共 7 条。
> **小屏**：流程横向滚动，State 面板移到流程图下方。
