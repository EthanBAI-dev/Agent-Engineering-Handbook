...
> **配图 F22-1｜扇出为 0 时下游整条被跳过**
> **形式**：对照图 · `assets/lessons/22-empty-fanout.svg`
> **画什么**：左栏三份文件——三个 map 任务汇入 reduce；右栏零份文件——map 一个都没有，
> reduce 节点整个画成虚线灰框，旁边标「没有报错，也没有执行」。
> **必须标注**：reduce 只有一条来自 map 的入边；右栏结果字段停在初始值。
> **不要出现**：在右栏画报错图标——它的危险正是不报错。
> **替代文本**：扇出为零时没有 map 任务，只依赖 map 的 reduce 节点也不会执行，结果字段静默保持初始值。
...
> **互动 U22-1｜处理三种数量**
> **状态**：待开发 · 建议 `apps/web/src/components/lesson-twentytwo-lab.tsx`
> **用户做什么**：把输入文件数在 0 / 3 / 50 之间切换，观察同一张图的行为。
> **屏幕上变什么**：0 份时 reduce 节点灰掉、报告字段留空并高亮「这就是 bug 的样子」；
> 50 份时显示本次的调用次数与估算成本。
> **通过条件**：让 0 份也给出说得通的结果，并说出 50 份时你会加什么限制。
> **小屏**：数量切换置顶，扇出示意改为纵向堆叠的缩略卡片。
