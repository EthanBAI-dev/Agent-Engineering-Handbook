...
> **互动 U22-1｜处理三种数量**
> **状态**：已实现 · `apps/web/src/components/lesson-twentytwo-article.tsx`（ScenarioLab）
> **用户做什么**：把输入文件数在 0 / 3 / 7 / 50 之间切换，并开关「空集合单独走一条路」。
> **屏幕上变什么**：map 任务数、reduce 有没有执行、报告字段、这一次的成本四行实时更新；
> 0 份且没有保护时报告字段停在 `''` 并标红。
> **通过条件**：让 0 份也给出说得通的结果。
> **小屏**：分段控件与开关纵向排列。
