...
> **画什么**：左轨「网页」——打开网址→点开实验→看图跑一遍；右轨「本地」——装 uv→进目录→`uv run`。
> 两轨在底部合成一个点：**同一个实验结果**。
> **必须标注**：网页轨标「不装环境／不需要 API Key」；本地轨标「终端／uv／Python」；会合点标「同一实验结果」。
> **不要出现**：云端沙箱、登录界面、付费墙——这些都还不存在。
> **替代文本**：网页路径和本地路径是两条并行的学习方式，最终得到同一个实验结果。
...
> **配图 F00-2｜终端里你看到的三样东西**
> **形式**：带标注的终端截图 · `assets/lessons/00-terminal-anatomy.png`
> **画什么**：一个真实终端窗口，用引线标出三处：提示符、当前目录、你输入命令的位置。
> **必须标注**：这三处各是什么；「命令在哪个目录执行，决定它找不找得到文件」。
> **不要出现**：伪造的输出。截图必须来自真的跑过的终端。
> **替代文本**：终端窗口由提示符、当前所在目录和输入区组成。
...
> **互动 U00-1｜五步流程预演 + 双轨步骤器**
> **状态**：已实现 · `apps/web/src/components/lesson-zero-lab.tsx`
> **用户做什么**：先点着走完一遍最小流程；再在「只用网页」和「本地运行」之间切换，逐条看步骤并复制命令。
> **屏幕上变什么**：当前角色高亮；切换轨道时步骤列表整组替换；每条命令有复制按钮。
> **通过条件**：走完五步，并能说出自己选的是哪条路径、下一步该做什么。
> **小屏**：流程节点横向滚动，步骤器改为纵向单列。
