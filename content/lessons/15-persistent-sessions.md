...
> **画什么**：上栏一个进程框，内存 checkpointer 画在框内，进程消失时里面的东西跟着消失；
> 下栏 checkpointer 画在框外的文件里，新进程只拿到一条文件路径就能读到。
> **必须标注**：「进程边界」那条线；下栏标「子进程没有继承任何内存」。
> **不要出现**：把 InMemorySaver 画成有持久化图标。
> **替代文本**：内存检查点随进程消失，外部存储写在进程之外，新进程只凭文件路径就能读回。
