...
> **画什么**：左边「多传的字段」——一个 `user_id` 从输入飘向 State 边界后消失；
> 右边「写错字段名」——节点返回 `answr`，同样在边界外消失。两处都画一个灰色的「无异常」标记。
> **必须标注**：两处都「没有报错、也没有生效」；State 边界画成一道实线。
> **不要出现**：红色报错图标——它们的危险恰恰在于不报错。
> **替代文本**：不在 schema 里的输入字段和写错名字的返回字段都会被静默丢弃，不产生任何错误提示。
...
> **互动 U12-1｜找出消失的字段**
> **状态**：待开发 · 建议 `apps/web/src/components/lesson-twelve-lab.tsx`
> **用户做什么**：跑三次调用，每次都有一个字段没按预期出现，选出原因。
> **屏幕上变什么**：State 面板用灰色幽灵条显示「被丢弃的字段」，并标出是在输入侧还是返回侧丢的。
> **通过条件**：三种原因分辨正确，并说出第三种（不在 `output_schema` 里）是设计而不是错误。
> **小屏**：三次调用改为可切换的标签页。
