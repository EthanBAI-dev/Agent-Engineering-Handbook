...
> **互动 U11-1｜让路由挡住三种坏输入**
> **状态**：已实现 · `apps/web/src/components/lesson-eleven-article.tsx`（ScenarioLab）
> **用户做什么**：切换模型这次的输出（合法枚举／大小写不同／一段自然语言），并开关白名单校验。
> **屏幕上变什么**：并列显示 `raw_route`（模型说了什么）与 `route`（程序信了什么）；关掉校验时后两种直接抛 `KeyError`。
> **通过条件**：非法输出被挡在控制流之外，落到兜底分支而不是让流程崩掉。
> **小屏**：两行结果纵向排列，标签保持可辨。
