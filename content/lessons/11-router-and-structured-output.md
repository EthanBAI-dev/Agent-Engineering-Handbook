...
> **画什么**：模型输出经过两道关卡才进入分支。第一道「结构化输出」标「更可能对」，
> 第二道「白名单校验」标「保证对」。三条示例输入从上方进入，非法的两条在第二道被拨到兜底分支。
> **必须标注**：两道关卡的性质差别；兜底分支标「最安全的那条」。
> **不要出现**：把第一道画成能单独挡住——那正是本课要纠正的。
> **替代文本**：模型的分类结果先经过结构化输出，再经过白名单校验，非法值一律落到最安全的兜底分支。
...
> **互动 U11-1｜让路由挡住三种坏输入**
> **状态**：待开发 · 建议 `apps/web/src/components/lesson-eleven-lab.tsx`
> **用户做什么**：切换模型输出（合法枚举／大小写不同的值／一段自然语言），看流程走向。
> **屏幕上变什么**：面板并列显示 `raw_route`（模型说了什么）与 `route`（程序信了什么）两列；
> 关掉校验开关后，后两种输入直接抛出 `KeyError`。
> **通过条件**：三种输入都没让流程崩掉，并说出兜底为什么选 `human` 而不是 `refund`。
> **小屏**：两列改为上下两行，保持标签可辨。
