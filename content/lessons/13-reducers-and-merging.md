...
> **互动 U13-1｜给字段配一条合并规则**
> **状态**：已实现 · `apps/web/src/components/lesson-thirteen-article.tsx`（ScenarioLab）
> **用户做什么**：切换执行方式（顺序／并行）与合并规则（覆盖／operator.add／自定义）。
> **屏幕上变什么**：并行 + 覆盖时直接给出 `InvalidUpdateError`；加上 reducer 后两条结果都在，并提示顺序不可靠。
> **通过条件**：三种规则都试过，并说出「覆盖」为什么只在有先后时才成立。
> **小屏**：两组分段控件与结果面板纵向堆叠。
