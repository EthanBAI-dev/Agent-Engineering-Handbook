...
> **术语说明**
>
> - **ToolNode**：执行模型已经提出的工具调用，不替模型理解用户。
> - **thread_id**：定位一次会话，不代表登录身份或访问权限。
...
> **术语说明**
>
> - **thread**：区分不同会话中的消息和状态。
> - **API Key（接口密钥）**：验证模型调用权限，必须只保存在服务端。
...
> **术语说明**
>
> - **TypedDict**：声明 State 必须有哪些字段。
> - **Annotated**：为字段附加类型之外的规则。
> - **add_messages**：把新消息追加到 messages（消息列表），而不是覆盖旧消息。
...
> **术语说明**
>
> - **root_path**：限制 Agent 能操作的测试目录。
> - **pending_action**：保存正在等待审批的动作。
> - **result**：保存动作执行或取消后的结果。
...
> **术语说明**
>
> - **compile**：把节点、Edge 和保存机制组合成可运行的 Agent 应用。
> - **checkpointer**：保存图的 checkpoint，让暂停后的流程能够找到原现场。
...
> **术语说明｜InMemorySaver**  
> 把 checkpoint 临时放在当前进程内存中，只适合教学和测试。
...
> **术语说明**
>
> - **Python**：运行服务端的 LangGraph 和真实工具代码。
> - **/api/agent**：接收网页请求，并返回 Agent 的运行结果。
