...
> **互动 U05-1｜把打乱的卡片排回去**
> **状态**：待开发 · 建议 `apps/web/src/components/lesson-five-lab.tsx`
> **用户做什么**：拖动打乱的节点卡片排成执行路径，并给必须审批的动作打上标记。
> **屏幕上变什么**：排对的相邻两张之间出现实线箭头，排错出现红色虚线并提示原因；
> 审批标记打在安全动作上时提示「审批要稀缺」。
> **通过条件**：路径包含 `model ⇄ tools`、删除动作前有 approval、拒绝后有安全结束；且只标了危险动作。
> **小屏**：卡片改为单列，用上移／下移按钮代替拖动。
