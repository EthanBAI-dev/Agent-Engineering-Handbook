...
> **术语说明｜Agent（智能体）**  
> 根据目标提出下一步动作；是否允许执行危险动作仍由图和人类审批控制。
...
> **术语说明**
>
> - **LangGraph**：在暂停后保留可继续运行的流程结构。
> - **checkpoint**：保存申请暂停时的现场。
> - **resume**：携带审批人的决定来恢复流程。
...
> **术语说明**
>
> - **propose**：说明 Agent 打算做什么。
> - **approval**：把问题交给人类并等待决定。
> - **report**：在流程恢复后整理并输出结果。
...
> **术语说明｜TypedDict**  
> 声明 State 必须包含哪些字段以及字段类型。

State 中的 `path`（文件路径）和 `result`（处理结果）记录了这次审批的数据。

> **术语说明**
>
> - **State**：让暂停前后的节点共享同一份现场。
> - **path**：保存准备处理的文件路径。
> - **result**：保存批准或拒绝后的处理结果。
...
> **术语说明**
>
> - **compile**：把节点、边和保存机制组合成可运行的应用。
> - **InMemorySaver**：把教学实验的 checkpoint 临时保存在当前进程中。
...
> **术语说明｜API Key**  
> 验证程序是否有权调用模型服务；本课没有模型调用，所以不需要配置。
