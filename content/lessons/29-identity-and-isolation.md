...
> **配图 F29-1｜thread_id 不是身份**
> **形式**：对照图 · `assets/lessons/29-ownership-check.svg`
> **画什么**：上栏没有归属检查——bob 拿着 `t_alice` 直接取到了 alice 的消息；
> 下栏加了归属检查——服务端的归属表挡在数据库之前，两种失败都回同一句「找不到这个会话」。
> **必须标注**：归属表标「来自服务端会话，不来自请求参数」；两种拒绝回同一句话。
> **不要出现**：把 `thread_id` 画成一把钥匙——它只是一个编号。
> **替代文本**：没有归属检查时凭编号就能读到别人的会话，加上服务端归属表后越权和不存在都返回同一句话。
