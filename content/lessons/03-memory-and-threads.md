...
State 保存当前现场；checkpoint 记录某一时刻的 State；checkpointer 存取 checkpoint；`thread_id` 把记录归到不同会话；LangGraph 按图运行节点并协调保存和恢复。
...
Agent 根据消息选择步骤、调用工具，并结合已有状态继续行动。

第 02 课已经把所有消息放进 `MessagesState`（消息状态）。

`MessagesState` 在一次图运行中持续累积对话和工具消息。
...
> **配图 F03-1｜checkpointer 存的是快照，不是聊天记录**
> **形式**：对照图 · `assets/lessons/03-checkpoint-vs-log.svg`
> **画什么**：左边一条「聊天记录」——只有几句文本；右边一个「checkpoint」——一个方框，
> 里面除了 messages，还有「下一步该跑哪个节点」和元数据。
> **必须标注**：右侧那两项额外内容；一句「所以它能继续运行，而不只是能被读」。
> **不要出现**：数据库表格图标——这一课的 InMemorySaver 只在内存里。
> **替代文本**：checkpoint 不只保存对话文本，还保存当时的完整状态和继续运行所需的位置信息。
...
`app.invoke` 把输入和会话配置交给整张图，并运行到结束或暂停位置。
...
> **互动 U03-1｜把三条消息分配到正确的 thread**
> **状态**：已实现 · `apps/web/src/components/lesson-three-lab.tsx`
> **用户做什么**：为每条消息选一个 thread，再调用图；跑完可重新分配再来一次。
> **屏幕上变什么**：两列 thread 同屏，目标列高亮；每次调用显示恢复了几条、model 看到几条输入、
> checkpoint 到第几版；下方 `get_state` 面板可切换查看任一 thread 的快照。
> **通过条件**：三条分别落在 a / b / a，最后一次提问看得见名字、看不见咖啡。
> **小屏**：组稿区与两列 thread 纵向堆叠，thread 列不设最大高度。
...
`get_state` 取出指定会话最近保存的 State，供程序或开发者检查。
...
> **配图 F03-2｜三种「记忆」不是一回事**
> **形式**：分层对照图 · `assets/lessons/03-memory-layers.svg`
> **画什么**：三层横条，从上到下：这一次调用带上哪些消息 / 这段对话说了什么 / 这个人是谁。
> 每层右侧标负责的组件和对应课号（第 14 课 / 本课 / 第 30 课）。
> **必须标注**：本课只解决中间那层；上下两层各属于哪一课。
> **不要出现**：把三层画成互相替代的方案——它们是叠加的。
> **替代文本**：上下文管理、会话记忆和长期记忆是三层不同的机制，本课只解决中间的会话记忆。
