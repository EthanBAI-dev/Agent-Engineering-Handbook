...
> **画什么**：三层环。最外层「标注来源」画成虚线并标「劝阻」；中间层「工具清单」画成实线并标「阻止」；
> 最内层「人工审批」画成实线并标「兜底」。一支写着「忽略之前的指令」的箭头穿过外层，被中层挡住。
> **必须标注**：三层各自的性质（劝阻／阻止／兜底）；箭头停在中层。
> **不要出现**：把外层画成能挡住——它和攻击者在同一个层面竞争。
> **替代文本**：提示词层面的标注只能劝阻，工具清单才能真正阻止，人工审批是最后一道兜底。
...
> 三层全开时停在审批界面。
> **通过条件**：说出单独第一层为什么不够，以及第二层挡不住什么。
> **小屏**：三个开关横向排列，注入内容与结果纵向堆叠。
