...
- 在同一句或紧接着的下一句回答“它是做什么的”，不能只给字面翻译。
- 同一段首次出现多个术语时，可以合并成一个“其中……”句；句子过长时拆成两句，不恢复成连续提示框。
- 代码、命令、字段名和函数名保持原样；如果它们首次出现在代码块中，在代码块后的普通正文补中文含义和作用。
- 品牌名、文件名、路径和普通产品名不强行翻译。
- 非归属课可以给简短中文提示，但不能借术语解释重新完整教授其他课拥有的概念。
- 旧的段后引用框版本已备份到 Git 分支 `backup/00-05-quote-terminology-2026-09-10`；效果对比见 `terminology-style-comparison.md`。

### 版式：一条主栏，一处刻意加宽

课页里所有直接子块共用同一条版心 `--measure`（710px），只有互动实验用 `--measure-wide`（920px）
向两侧对称展开。规则写在 `.chapter.lesson-article > *` 上，不靠每个区块各自记得。

改这条之前，正文在 710、其余区块在 920，往下读时左边缘在 420 与 525 之间来回跳十几次。
两个宽度本身没问题，问题是切换没有理由——现在一页只切换一次，而且发生在实验区。

配套两条：

- 大标题的字号要能容下作者写的换行。收窄版心后 82px 会把标题挤成 3–4 行并甩出孤字，
  已降到 `clamp(40px, 5vw, 68px)` 并加 `text-wrap: balance`。
- `.site-shell` 加 `overflow-x: clip`。术语浮层即使隐藏也占布局，靠右的那些会把整页撑宽；
  `clip` 不像 `hidden` 会创建滚动容器，顶栏与侧栏的 sticky 不受影响。

### 精简版与完整版

两个版本都保留，并且是产品的两档：

- **精简版**（`/lessons/<slug>/brief`）：免费。互动实验与完整版**完全一样**，只是正文更短。
- **完整版**（`/lessons/<slug>`）：付费方向。多出真实代码逐段拆解、边界条件、常见误区、
  排查清单和本地运行说明。

目前 00–03 两版都有，04 起只写完整版；`BRIEF_LESSONS` 控制哪些课有精简版，
没有的直接 404，不会落到别人的正文上。

**收费本身还没有做**，需要先定：账号体系放在哪一层、按课买还是订阅、
精简版是否要被搜索引擎收录（现在 canonical 指向完整版）。这些决定之前不要先写支付代码。

### 网页中的渐进式术语解释

Markdown 原稿使用同句或下一句直接解释，保证脱离网站、关闭 JavaScript 或直接在 GitHub 阅读时，解释仍然完整。网页可以把同一类解释改成渐进披露：正文中的关键词保留中文含义并加虚线下划线，鼠标悬停、键盘聚焦或手机点击后显示“它是做什么的”。

- 组件必须支持 hover、focus 和 click，不能只照顾鼠标。
- 触发词使用按钮语义和 `aria-describedby`；浮层使用 `role="tooltip"`。
- 再次点击、移走焦点或按 Escape 后关闭，正文布局不能因浮层出现而跳动。
- **每一次出现都给提示**（2026-09-10 按学习者反馈修改，取代原来的「只解释第一次」）：同一个术语在正文里重复出现时，读者同样可能已经忘了，往回翻的成本比多一条虚线更高。
- 术语解释集中在 `apps/web/src/lib/glossary.ts`，正文由 `auto-term.tsx` 自动标注，作者不再手工包裹每一处。
- 自动标注按最长优先匹配，并跳过代码块、示意图和标题：大号展示字加虚线下划线会毁掉版式，代码里加按钮语义则会干扰阅读。
