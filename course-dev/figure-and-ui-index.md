...
当前共 **37 张配图**（全部待制作）、**31 个互动实验**（31 个已实现，0 个待开发）。
...
| `U26-1` | 26 | 三个问题，三种结果 | 已实现 · `apps/web/src/components/lesson-twentysix-article.tsx`（ScenarioLab） |
| `U27-1` | 27 | 找出被弄坏的那一条 | 已实现 · `apps/web/src/components/lesson-twentyseven-article.tsx`（ScenarioLab） |
| `U28-1` | 28 | 三种注入，三层防线 | 已实现 · `apps/web/src/components/lesson-twentyeight-article.tsx`（ScenarioLab） |
| `U29-1` | 29 | 用两个账号试一次 | 已实现 · `apps/web/src/components/lesson-twentynine-article.tsx`（ScenarioLab） |
| `U30-1` | 30 | 三层记忆各管一段 | 已实现 · `apps/web/src/components/lesson-thirty-article.tsx`（ClassifyLab） |
